...
class PDF extends FPDF
{
		...
		$asignaciones = mysqli_query($conex,"SELECT concat(estudiantes.NombresEstudiante,' ',estudiantes.ApellidosEstudiante) as Estudiante, asignaturas.NombreAsignatura as Asignatura, entrega_tareas.Descripcion as Descripcion, entrega_tareas.Archivo as Archivo, entrega_tareas.FechaEntrega as Fecha FROM asignaciones INNER JOIN asignaturas ON  asignaciones.idAsignatura =  asignaturas.idAsignatura 
                                         INNER JOIN entrega_tareas ON  asignaturas.idAsignatura =  entrega_tareas.idAsignatura 
										 INNER JOIN estudiantes ON  entrega_tareas.idEstudiantes =  estudiantes.idEstudiantes
where asignaciones.idProfesor = '$codigo' and asignaturas.NombreAsignatura = '$Asignatura' group by estudiantes.idEstudiantes");
