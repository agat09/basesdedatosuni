...
function Header()
{
	...
	$this->Cell(160);
	$this->Cell(50,20,'Tareas Orientadas por Profesor',0,0,'C');
	$this->Ln(15);
	...
	$this->SetFont('Arial','B',12);
		    $this->Cell(160,20,'Asignaciones del Profesor:',0,0,'R');
		    $codigo = $_SESSION["Codigo"];
            $asignaciones = mysqli_query($conex,"SELECT concat(NombresProfesor, ' ' ,ApellidosProfesor) as Profesor FROM profesor where idProfesor= '$codigo'");
...
from planificacion_tareas inner join numeros_asignaciones on planificacion_tareas.idNumeroAsignacion=numeros_asignaciones.idNumeroAsignacion 
  ...
  WHERE planificacion_tareas.idProfesor = '$codigo'");
$item = 0;
while($tareas2 = mysqli_fetch_array($tareas)){
	$item = $item+1;
